...
int readSingleRow(FILE *f, char *toSave, int dim, char discerning) {
    ...
    char *temp = (char*)malloc(sizeof(char) * dim + 1);
    //printf("Ho una stringa di dim %ld che vale: [%s] dim: %d\n", sizeof(temp), temp, dim);

    fgets(temp, dim, f);
    for (int i=0; i<dim; i++) {
        if (temp[i] == discerning) {
            while (fscanf(f, "%c", &temp[i]) > 0 && temp[i] != '\n');         //consumiamo la riga rimasta per far muovere il descrittore'f'
            temp[i]='\0';
            break;
        }
    }
