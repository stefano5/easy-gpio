...
void write_generic_log(char *text) {
	...
	char * toWrite = (char*)malloc(strlen(text) + strlen(getTime()) + 4);
...
/*	
 *	check path: /sys/class/gpio/gpioN/value. if not exist then return 0, else 1
*/
 ...
 *	
 *	At the end the the pin will be initialized and it will be possible to use it correctly
 *
...
/*	turn on/off pin		and, if it is necessary, initialize automatically the pin
 *
...
/* 
 * @return	0 if the gpio is "in"
 * 		    1 if tue gpio is "out"
...
int read_direction(int pin) {
	...
	//read direction without initilization is not possible
...
/*
 *	For example see down: f_pwm_gpio()
 *
